...
@section('page-title', 'Partner Section')
...
@section('breadcrumbs')
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-start mb-0">Partner Section</h2>
                <div class="breadcrumb-wrapper">
                    {{ Breadcrumbs::render('our-number.partners') }}
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="row">
                <div class="col-md-12 col-12">
                    <div class="card">
                        <div class="card-body">
                            {{-- Form Start--}}
                            <form id="updateNumberPartnerSection" enctype="multipart/form-data">
                                @csrf
                               {{-- @dd($partner) --}}
                               <div class="row mb-2">
                                <div class="col-lg-6 col-md-6 col-12">
                                    <label class="form-label fs-5" for="basic-addon-name"> Heading </label>
                                    <input type="text" id="partner_heading_en" class="form-control"
                                        name="partner_heading_en" placeholder="Heading" aria-label="Name"
                                        aria-describedby="basic-addon-name" value="{{ $our_partners ? $our_partners->fieldSingleValue('partner_heading','en')->value : ''}}" required />
                                </div>

                                <div class="col-lg-6 col-md-6 col-12">
                                    <label class="form-label fs-5" for="basic-addon-name">  قصة العنوان </label>
                                    <input type="text" name="partner_heading_ar" id="partner_heading_ar"
                                        class="form-control" placeholder="قصة عنوان " aria-label="Name"
                                        aria-describedby="basic-addon-name" value="{{ $our_partners ? $our_partners->fieldSingleValue('partner_heading','ar')->value : '' }}" required />
                                </div>
                            </div>
                            ...
                            <div class="row mb-2">
                                <div class="col-lg-6 col-md-6 col-12">
                                    ...
                                    <textarea class="form-control form-control-lg @error('description') is-invalid @enderror description" rows="12"
                                        name="partner_description_en" id="partner_description_en">  {{ $our_partners ? $our_partners->fieldSingleValue('partner_description','en')->value : '' }}
                                    </textarea>
                                </div>
                                ...
                                <div class="col-lg-6 col-md-6 col-12">
                                    ...
                                    <textarea class="form-control form-control-lg @error('description') is-invalid @enderror description" rows="12"
                                        name="partner_description_ar" id="partner_description_ar">  {{ $our_partners ? $our_partners->fieldSingleValue('partner_description','ar')->value : '' }}
                                    </textarea>
                                </div>
                            </div>
...
<script>
        ...
        $(document).on('submit', '#updateNumberPartnerSection', function(e) {
            e.preventDefault();
            $('.update_button').hide();
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('our-numbers.numberPartnerSection.update') }}",
                data: formData,
                dataType: "json",
            ...
            FilePond.create(document.getElementById('image'), {
                ...
                files: [
                @forelse ($partner_images as $image)
                    {
                        source: "{{ asset('app-assets/images/numbers/partners' . '/' . $image->image) }}",
                    },
                @empty
                @endforelse
                ],
