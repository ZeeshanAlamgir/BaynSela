...
@section('content')
    <form method="post" action="{{ route('user-list.user.store') }}">
    @csrf
        <div class="card">
            <div class="card-body">
            {{ view('app.admin-panel.users.form-fields',compact('data','is_password')) }}
            </div>
...
@section('custom-js')
<script>
    $("#project_type_id").select2({ maximumSelectionLength: 5 });
    $("#project_duration_id").select2({ maximumSelectionLength: 5 });
</script>
@endsection
