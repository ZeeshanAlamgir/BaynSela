...
if (!function_exists('updateMultiValue')) {
    function updateMultiValue($model, $data)
    {
        foreach ($data as $key => $value) {
            $model->multiValues()->update([
                'field' => $value['field'],
                'value' => $value['value'],
                'locale' => $value['locale'],
            ]);
...
if (!function_exists('get_default_service')) {
    function get_default_service()
    {
        $service = (new Service())->orderBy('id', 'asc')->get()->first();
        return $service->id;
    }
}

if (!function_exists('is_event_has_filter_value')) {
    function is_event_has_filter_value($event_id,$filter_id,$filter_value_id)
    {
        $event_filter = (new EventFilter())->where('event_id',$event_id)->where('filter_id',$filter_id)->where('filter_value_id',$filter_value_id)->get()->first();
